...
async function generateStagedDynamicFlightRenderResultNode(
  ...
  // Runtime-cache this navigation. Partial Prefetching is part of the fork's
  // rendering model, so the dynamic render always fills caches before spawning
 ...
 * retain `instrumented`, not request-specific data like req/res/workStore.
 */
...
export async function runValidationInDevFromSnapshot(
  ...
  if (componentMod.__next_app__) {
    installGlobalModuleLoadingHandlers(componentMod, false)
  }
...
async function validateInstantConfigs(
  ...
  async function validateAtDepthImpl(
    ...
    const dynamicHoleKind = payloadResult.hasAmbiguousErrors
  ...
  const prefetchKind = ValidationPrefetchKind.Shell
...
async function renderWithRestartOnCacheMissInValidation(
  ...
  const shouldRenderAppShell = true
...
async function prerenderToStream(
  ...
  try {
    ...
    let { prelude: unprocessedPrelude, postponed } = await runInSequentialTasks(
      () => {
        const stream =
          process.env.NODE_ENV === 'development' && collectedChunks.allChunks
            ? createNodeStreamWithLateRelease(
                collectedChunks.prerenderChunks,
    ...
    if (!isHTTPAccessFallback && !isRedirect) {
      ...
      runInSequentialTasks(
        ...
        () => {
          if (!errorServerReactController.signal.aborted) {
            workUnitAsyncStorage.run(
              errorPrerenderStore,
              errorServerReactController.abort.bind(errorServerReactController)
            )
          }
        }
      )
    ...
    try {
      ...
      const { prelude: unprocessedErrorHtmlStream, postponed: errorPostponed } =
        await runInSequentialTasks(
          () => {
...
async function collectSegmentData(
  ...
  if (renderOpts.isBuildTimePrerendering) {
    ...
    // read here. The prerender tracks accesses and stores the settled hint.
    const hintCell =
    ...
    if (manifestHints === undefined) {
      if (!prefetchInlining) {
        // With inlining disabled, a missing entry means the route did not
        // qualify for the static-prefetch hint at build. Skip the hint system
        // and let the client proceed directly to runtime prefetching.
        hints = null
      } else {
